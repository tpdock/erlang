...
RUN kerl build R16B03-1 R16B03-1; kerl install R16B03-1 /usr/lib/erlang/R16B03-1; kerl cleanup R16B03-1; rm /root/.kerl/archives/*.tar.gz
...
RUN kerl build 18.1 18.1; kerl install 18.1 /usr/lib/erlang/18.1; kerl cleanup 18.1; rm /root/.kerl/archives/*.tar.gz
RUN kerl build 18.2 18.2; kerl install 18.2 /usr/lib/erlang/18.2; kerl cleanup 18.2; rm /root/.kerl/archives/*.tar.gz
RUN kerl build 18.2.1 18.2.1; kerl install 18.2.1 /usr/lib/erlang/18.2.1; kerl cleanup 18.2.1; rm /root/.kerl/archives/*.tar.gz
RUN kerl build 18.3 18.3; kerl install 18.3 /usr/lib/erlang/18.3; kerl cleanup 18.3; rm /root/.kerl/archives/*.tar.gz
RUN kerl build 19.0 19.0; kerl install 19.0 /usr/lib/erlang/19.0; kerl cleanup 19.0; rm /root/.kerl/archives/*.tar.gz

ADD /tools/prepare.sh /root/prepare.sh
RUN /root/prepare.sh

ENV _KERL_PATH_REMOVABLE "/usr/lib/erlang/19.0/bin"
